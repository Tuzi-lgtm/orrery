/**
 * Procedural planet surfaces, painted pixel by pixel on a canvas.
 *
 * Deliberately free of any `three` import so it can be pulled into the texture
 * worker without dragging the renderer along, and it draws on an OffscreenCanvas
 * when no DOM is around. `textures.ts` wraps the output in three Textures.
 */
export type PaintedCanvas = HTMLCanvasElement | OffscreenCanvas;

export type TextureKind = "planet" | "bump" | "cloud" | "moon";
...
/** How each painted kind must be sampled once it becomes a three Texture. */
export const TEXTURE_OPTS: Record<TextureKind, TextureOpts> = {
  planet: {},
  bump: { srgb: false },
  cloud: {},
  moon: {},
...
function createCanvas(w: number, h: number): PaintedCanvas {
...
function context2d(canvas: PaintedCanvas) {
...
/** Wrap painted pixels in a canvas. Main thread only — three needs a source. */
export function canvasFromImageData(img: ImageData): PaintedCanvas {
  const canvas = createCanvas(img.width, img.height);
  context2d(canvas).putImageData(img, 0, 0);
  return canvas;
}

export function paintGlow(): PaintedCanvas {
  const size = 512;
...
function planetPixels(id: string, seed: number): ImageData {
  return paint(1024, 512, (u, v, p) => {
    switch (id) {
      case "mercury":
...
function bumpPixels(id: string, seed: number): ImageData {
...
function cloudPixels(seed: number): ImageData {
  return paint(1024, 512, (_u, v, p) => {
...
export function paintRing(kind: "saturn" | "uranus"): PaintedCanvas {
  const canvas = createCanvas(8, 1024);
  const ctx = context2d(canvas);
...
function moonPixels(seed: number): ImageData {
  return paint(512, 256, (_u, _v, p) => {
...
/**
 * Single entry point for the worker protocol: one kind + one seed in, one
 * buffer of pixels out.
 *
 * Deliberately pixels and not an ImageBitmap. A bitmap transfers zero-copy,
 * but WebGL will not apply UNPACK_FLIP_Y_WEBGL to one, so a bitmap-backed
 * texture samples vertically mirrored against the canvas-backed texture this
 * replaced — every planet upside down. Handing back raw pixels lets the main
 * thread build the same CanvasTexture as before, orientation included.
 */
export function paintTexture(
  ...
  switch (kind) {
    case "planet":
      return planetPixels(body, seed);
    case "bump":
      return bumpPixels(body, seed);
    case "cloud":
      return cloudPixels(seed);
    case "moon":
      return moonPixels(seed);
  }
}
